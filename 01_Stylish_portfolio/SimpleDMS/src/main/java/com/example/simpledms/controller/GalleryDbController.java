...
import com.example.simpledms.dto.galleryDb.ResponseFileDto;
...
public class GalleryDbController {
    ...
    public ResponseEntity<Object> galleryDbUploadFile(@RequestParam("galleryTitle") String galleryTitle,
...
//          Upload 이미지 정보를 가져오는 함수
            Page<ResponseFileDto> galleryPage = galleryDbService
...
//                        modelMapper.map(소스모델, 타겟DTO.class)
                        com.example.simpledms.dto.galleryDb.ResponseFileDto galleryDto = modelMapper.map(dbGallery, com.example.simpledms.dto.galleryDb.ResponseFileDto.class);
...
//        id 로 조회 함수
        GalleryDb galleryDb = galleryDbService.getFile(gid).get();
...
//        첨부파일 다운로드 : url Content-Type 규칙
    ...
    public ResponseEntity<Object> deleteGalleryDb(@PathVariable int gid) {
